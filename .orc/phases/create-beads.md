...
- Interfaces (provider.go, store.go) before implementations (local.go, sqlite.go, ecs.go).
- Internal packages before CLI commands that call them.
- Config/store before provider, provider before CLI.
