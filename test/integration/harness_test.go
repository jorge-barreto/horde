...
func TestMain(m *testing.M) {
	...
	// The ECS-backed tests run against the CDK stack, which is managed
	// externally via `make e2e-up` / `make e2e-down` (deploy, populate
	// secrets, push the worker image, then destroy). TestMain does not
	// deploy or destroy any stack — the ECS tests skip-gate themselves on
	// HORDE_E2E_ECS / HORDE_E2E_ECS_BACKEND=cdk and the CDK state file.
